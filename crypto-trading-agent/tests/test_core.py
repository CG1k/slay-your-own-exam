...
def test_immediate_execution_when_confirmation_off(store, clock):
    core, exchange, _ = make_core(store, clock)
    out = core.submit_prediction("BTC-USD", "rise", 0.9, 24, THESIS)
...
def test_low_confidence_records_but_never_proposes(store, clock):
    core, _, _ = make_core(store, clock)
    out = core.submit_prediction("BTC-USD", "rise", 0.5, 24, THESIS)
...
def test_fall_sells_held_position(store, clock):
    core, exchange, _ = make_core(store, clock)
    core.submit_prediction("BTC-USD", "rise", 0.95, 24, THESIS)
    clock.advance(7 * 3600)  # past conservative cooldown (6h)
    out = core.submit_prediction("BTC-USD", "fall", 0.95, 24, THESIS)
    assert out["executed"] is True
...
def test_kill_switch_blocks_and_reenables(store, clock):
    ...
    out = core.submit_prediction("BTC-USD", "rise", 0.95, 24, THESIS)
    assert out["executed"] is False
    with pytest.raises(ValueError):
        core.close_position("BTC-USD", 1.0, "should fail: nothing held")
    core.set_trading_enabled(True)
    assert core.submit_prediction("ETH-USD", "rise", 0.95, 24, THESIS)["executed"] is True
...
def test_challenge_unwinds_buy(store, clock):
    core, exchange, _ = make_core(store, clock)
    buy = core.submit_prediction("BTC-USD", "rise", 0.95, 24, THESIS)
...
def test_stop_loss_maintenance(store, clock):
    ...
    core.submit_prediction("BTC-USD", "rise", 0.95, 24, THESIS)
    prices["BTC-USD"] = 50_000.0 * 0.90  # -10%, past conservative stop of 5%
...
def test_daily_loss_breaker_disables_trading(store, clock):
    ...
    out = core.submit_prediction("ETH-USD", "rise", 0.95, 24, THESIS)
    assert out["executed"] is False
...
def test_input_validation(store, clock):
    core, _, _ = make_core(store, clock)
    with pytest.raises(ValueError):
        core.submit_prediction("btc_usd!!", "rise", 0.9, 24, THESIS)
    with pytest.raises(ValueError):
        core.submit_prediction("BTC-USD", "moon", 0.9, 24, THESIS)
    with pytest.raises(ValueError):
        core.submit_prediction("BTC-USD", "rise", 1.5, 24, THESIS)
    with pytest.raises(ValueError):
        core.submit_prediction("BTC-USD", "rise", 0.9, 0.1, THESIS)
    with pytest.raises(ValueError):
        core.submit_prediction("BTC-USD", "rise", 0.9, 24, "gm")
